...
void print_help() {
    ...
    std::println("      --refs-to-loose VA  superset of --refs-to. Also catches the fn-pointer-only");
    std::println("                       case (Roblox-style obfuscation) by scanning:");
    std::println("                         (a) every readable section for VA as a literal qword");
    std::println("                             — `imm64-stored` rows;");
    std::println("                         (b) ELF R_*_RELATIVE relocations whose addend == VA, the");
    std::println("                             post-load value of `.data.rel.ro` slots that read as");
    std::println("                             zero on disk — `relocated` rows.");
    std::println("                       Each row carries the slot address and (when known) the");
    std::println("                       containing function so an agent can chase the dispatch.");
    ...
    std::println("      --annotate ADDR  append one annotation to the resolved file (same precedence");
    std::println("                       as --apply). Use with --set-name / --set-note / --set-signature,");
    ...
    std::println("                       (rename / note / signature) with its meta. TSV by default,");
    std::println("                       structured form under --json. Use this to enumerate notes");
    std::println("                       — `--functions --json` only surfaces named records.");
    std::println("      --disasm-window VAs  batch sibling of --disasm-at. Comma-separated VAs (or");
    std::println("                       @PATH for one-per-line file input). Each block is preceded");
    std::println("                       by `# <hex-va>` so a splitter can pull them apart; --json");
    std::println("                       returns an array of [addr, disasm] objects. --count N caps");
    std::println("                       insns per VA (default 32). One-shot for thousands of hits.");
