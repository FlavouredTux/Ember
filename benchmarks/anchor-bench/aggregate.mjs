...
const entries = [...byKey.values()].map((rs) => {
  ...
  return {
    ...
    cost_usd: mean(rs.map((r) => r.totals.cost_usd ?? r.agent?.cost_usd ?? 0)),
    latency_s: mean(rs.map((r) => r.totals.latency_s ?? r.agent?.latency_s ?? 0)),
    spawned: mean(rs.map((r) => r.agent?.spawned ?? 0)),
    rejected: mean(rs.map((r) => r.agent?.rejected ?? 0)),
    claims_filed: mean(rs.map((r) => r.agent?.claims_filed ?? 0)),
    low_conf_name_claims: mean(rs.map((r) => r.agent?.low_conf_name_claims ?? 0)),
    retry_skipped: mean(rs.map((r) => r.agent?.retry_skipped ?? 0)),
    consensus_escalated: mean(rs.map((r) => r.agent?.consensus_escalated ?? 0)),
    coverage: mean(rs.map((r) => r.calibration?.coverage ?? 0)),
    high_conf_precision: mean(rs.map((r) => r.calibration?.high_conf_precision ?? 0)),
    low_conf_precision: mean(rs.map((r) => r.calibration?.low_conf_precision ?? 0)),
    abstention_accuracy: mean(rs.map((r) => r.calibration?.abstention_accuracy ?? 0)),
    ...
    reports: rs.map((r) => ({
      ...
      cost_usd: r.totals.cost_usd ?? r.agent?.cost_usd ?? 0,
      latency_s: r.totals.latency_s ?? r.agent?.latency_s ?? 0,
      claims_filed: r.agent?.claims_filed ?? 0,
      consensus_escalated: r.agent?.consensus_escalated ?? 0,
      high_conf_precision: r.calibration?.high_conf_precision ?? 0,
      coverage: r.calibration?.coverage ?? 0,
    })),
    slices: first.slices,
  };
});
